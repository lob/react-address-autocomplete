...
```
# At the parent level where @lob/react-address-autocomplete lives
npm link
# Go to demo project
cd example/
# Complete link to local copy
npm link @lob/react-address-autocomplete
```

Now when we import anything from @lob/react-address-autocomplete, npm will use the local copy of the package on our system. Please note that you must run `npm run build` to apply changes made to the local copy of @lob/react-address-autocomplete. `npm run start` will automatically rebuild any changes made to the repo for a smoother developer experience.

When you are done testing changes, use `npm unlink` in the reverse order.
